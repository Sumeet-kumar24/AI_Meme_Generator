...
async function parseJsonResponse(res, label = 'API response') {
...
async function fetchMemes() {
  ...
  for (const url of endpoints) {
    try {
      ...
      const data = await parseJsonResponse(res, url.includes('imgflip.com') ? 'Imgflip API' : 'Backend API');

      if (data?.success === false) {
        throw new Error(data.error || data.error_message || 'API returned failure');
      }

      const memes = data?.data?.memes;
      if (!Array.isArray(memes)) {
        throw new Error('Template payload was missing the meme list.');
      }

      // Filter out blocked templates
      state.allMemes = memes.filter(m =>
      ...
      lastError = err;
      console.warn(`Template fetch failed for ${url}:`, err);
    }
  }

  templateGrid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:40px;color:#ef4444">
      ⚠️ Could not load templates. Check your internet connection.<br><small>${lastError ? lastError.message : 'Unknown error'}</small>
    </div>`;
  console.error('Imgflip API error:', lastError);
}
